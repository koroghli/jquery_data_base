...
$request="SELECT game.id AS game_id, game.name game_name, game.year game_year,
...
$getAllGamesName= selectDataIntoDb($request,$db);
...
<html>
  ...
  <body>
    ...
    if (isset($_SESSION['error_message'])) {
      ...
      unset($_SESSION['error_message']); // On suprime le message
    };
    //Message s'ffiche si le jeu est correctement inserré.
...
<div class="row" id="ligne4">
  <div class="col-sm-8">
    <form action="vagrantDestroy.php" method="get">
      <select  name="destroy_select">

        <?php
while ($data=mysqli_fetch_assoc($getAllGamesName)) {
  echo "<option value = '" . $data['game_id'] . "'>" . $data['game_name'] . "</option>";
};

         ?>
      </select>
      <input type="submit" value="Supprimer" class="btn btn-default"style="background-color:#4a8297;border-color:#4a8297;color:white;">
    </form>
  </div>
</div>
<div class="row" id="ligne4">
